"""
PCO Helper Functions
Reusable utility functions for interacting with Planning Center Online API
"""
...
def add_person(pco: pypco.PCO, first_name: str, last_name: str, 
    ...
    Example:
        >>> pco = get_pco_client()
        >>> new_person = add_person(pco, "Jane", "Doe", gender="Female")
        >>> if new_person:
...
def update_person_attribute(pco: pypco.PCO, person_id: str, 
    ...
    Args:
        ...
        attribute_name: Name of the attribute to update (e.g., 'gender', 'birthdate')
    ...
    Example:
        >>> pco = get_pco_client()
        >>> updated = update_person_attribute(pco, "123456", "gender", "Male")
    """
    try:
...
def update_person_attributes(pco: pypco.PCO, person_id: str, 
    ...
    """
    Update multiple attributes for a person.
    ...
    Example:
        >>> pco = get_pco_client()
        >>> updated = update_person_attributes(pco, "123456", {
        ...     "gender": "Male",
...
def add_email_to_person(pco: pypco.PCO, person_id: str, 
    ...
    Args:
        ...
        location: Email location (Work, Home, etc.). Default: Work
    ...
    Example:
        >>> pco = get_pco_client()
        >>> email = add_email_to_person(pco, "123456", "john@example.com", "Work")
    """
    try:
        email_payload = {
            "data": {
...
def update_email(pco: pypco.PCO, person_id: str, email_id: str,
    ...
    """
    Update an email address for a person.
...
def create_or_update_person(pco: pypco.PCO, first_name: str, last_name: str,
    ...
    """
    Create a new person or update if they already exist.
...
def delete_person(pco: pypco.PCO, person_id: str) -> bool:
    ...
    Example:
        >>> pco = get_pco_client()
        >>> success = delete_person(pco, "123456")
    """
    try:
...
if __name__ == "__main__":
    ...
    # Example: Create or update a person with email
    person = create_or_update_person(
        pco,
        first_name="Test",
        last_name="User",
    ...
    if person:
        print(f"\nFinal person ID: {person['id']}")
        
        # Get all emails
        emails = get_person_emails(pco, person['id'])
        print(f"Person has {len(emails)} email(s)")
        for email in emails:
